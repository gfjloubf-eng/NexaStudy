...
const pendingAssignments = [
  { title: 'تقرير يومي عن المشروع', due: 'قبل 2 يوم' },
  { title: 'تسليم واجب البرمجة', due: 'قبل 5 أيام' },
]

const submittedAssignments = [
  { title: 'مراجعة الكتاب', status: 'مُرسل' },
  { title: 'ملخص الدرس', status: 'تمت مراجعة جزئية' },
]
  ...
  return (
    <div className="dashboard-shell" dir="rtl" lang="ar">
      <style>{`
        ...
        .assignment-card { padding: 16px; border-radius: var(--radius-sm); background: var(--color-surface-muted); display: grid; gap: 8px; }
        .assignment-card strong { color: var(--auth-text); }
        .assignment-card span { color: var(--auth-muted); }
      ...
      <div className="dashboard-layout">
        ...
        <main className="dashboard-main">
          ...
          <section className="page-header">
            <div>
              <h1>الواجبات</h1>
              <p>تابع الواجبات المعلقة والمُرسلة</p>
            </div>
            <Button variant="secondary">عرض الكل</Button>
          </section>

          <section className="section-card">
            <h2>الواجبات المعلقة</h2>
            <div className="card-grid">
              {pendingAssignments.map((assignment) => (
                <article key={assignment.title} className="assignment-card">
          ...
          <section className="section-card">
            <h2>الواجبات المرسلة</h2>
            <div className="card-grid">
              {submittedAssignments.map((assignment) => (
                <article key={assignment.title} className="assignment-card">
                  <strong>{assignment.title}</strong>
                  <span>{assignment.status}</span>
                  <Button variant="secondary">عرض الحالة</Button>
                </article>
              ))}
            </div>
          </section>
